...
class RepoEntitySearcher:
    ...
    def get_nodes_by_hunk(self, file_path: str, start_line: int, end_line: int) -> set:
        """
        根据文件路径和连续的行号范围（hunk），找到图中所有涉及的最具体节点集合。
        """
        hunk_nodes = set()

        # 优化：提前筛选出该文件的所有节点，避免每次循环都遍历全图
        file_nodes = []
...
def analyze_hunk_dependencies(G, hunks: List[tuple]):
    """
    分析多个 hunk（代码块）之间的依赖关系和层数。

    :param G: networkx 图对象 (MultiDiGraph)
    :param hunks: 列表，包含 (file_path, start_line, end_line) 元组
                  例如: [('src/main.py', 10, 20), ('src/utils.py', 42, 50)]
    :return: 依赖关系字典列表
    """
    searcher = RepoEntitySearcher(G)

    # 1. 将输入的 hunk 映射到图中的节点集合
    hunk_to_nodes = {}
    for hunk in hunks:
        file_path, start_line, end_line = hunk
        nodes = searcher.get_nodes_by_hunk(file_path, start_line, end_line)
        hunk_to_nodes[hunk] = nodes
        if not nodes:
            print(f"Warning: Could not find any nodes for hunk {file_path}:{start_line}-{end_line}")

    results = []
    hunk_list = list(hunk_to_nodes.keys())

    # 2. 计算两两 hunk 之间的最短依赖路径
    for i in range(len(hunk_list)):
        for j in range(len(hunk_list)):
            if i == j:
                continue

            src_hunk = hunk_list[i]
            tgt_hunk = hunk_list[j]
            src_nodes = hunk_to_nodes[src_hunk]
            tgt_nodes = hunk_to_nodes[tgt_hunk]

            if not src_nodes or not tgt_nodes:
                continue

            # 如果两个 hunk 映射出的节点集合有交集，说明它们在同一个函数/类内，依赖层数为0
            intersection = src_nodes.intersection(tgt_nodes)
            if intersection:
                shared_node = list(intersection)[0]
                results.append({
                    'source_hunk': src_hunk,
                    'target_hunk': tgt_hunk,
                    'source_node': shared_node,
                    'target_node': shared_node,
                    'has_dependency': True,
                    'hops': 0,
                    'path': [shared_node]
                })
            ...
            # 寻找从 src_nodes 集合 到 tgt_nodes 集合 的全局最短路径
            min_hops = float('inf')
            best_path = None
            best_src_node = None
            best_tgt_node = None
            ...
            for s_node in src_nodes:
                for t_node in tgt_nodes:
                    try:
                        # 计算单点到单点的最短路径
                        path = nx.shortest_path(G, source=s_node, target=t_node)
                        hops = len(path) - 1
                        ...
                        if hops < min_hops:
                            min_hops = hops
                            best_path = path
                            best_src_node = s_node
                            best_tgt_node = t_node
            ...
            if best_path is not None:
                results.append({
                    'source_hunk': src_hunk,
                    'target_hunk': tgt_hunk,
                    'source_node': best_src_node,
                    'target_node': best_tgt_node,
                    'has_dependency': True,
                    'hops': min_hops,
                    'path': best_path
                })
